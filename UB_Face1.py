...
def detect_faces(img: np.ndarray) -> List[List[float]]:
    ...
    img_grey_image=cv2.cvtColor(img,cv2.COLOR_BGR2GRAY)
    face_detect_haar = cv2.CascadeClassifier('haarcascade_frontalface_default.xml')

    detect_more_faces=face_detect_haar.detectMultiScale(img_grey_image,1.05,17,minSize=(32,32),flags=cv2.CASCADE_SCALE_IMAGE)

    for xaxis,yaxis,width,height in detect_more_faces:
        cv2.rectangle(img_grey_image,(xaxis,yaxis),(xaxis+width,yaxis+height),(29,29,29),2) 
        detection_results.append([float(xaxis),float(yaxis),float(xaxis+width),float(yaxis+height)])
